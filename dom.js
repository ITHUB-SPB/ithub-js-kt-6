export function renderTable(productsData) {
  ...
  tableBodyElement.append(
    ...productsData.map((productRecord) => {
      ...
      let averageMark = "—";
      let marksCount = 0;
      if (Array.isArray(productRecord.marks) && productRecord.marks.length > 0) {
        const sum = productRecord.marks.reduce((a, b) => a + b, 0);
        averageMark = (sum / productRecord.marks.length).toFixed(1);
        marksCount = productRecord.marks.length;
      }
